"""
This module provides classes and functions for coordinate conversion
between Miro and PowerPoint coordinate systems.
"""
...
class CoordinateConverter:  # pylint: disable=too-many-instance-attributes
    ...
    def miro_to_pptx_position(self, x: float, y: float,  # pylint: disable=too-many-arguments
    ...
    def get_text_box_position(self, miro_x: float, miro_y: float,  # pylint: disable=too-many-arguments
    ...
    def get_image_position(self, miro_x: float, miro_y: float,  # pylint: disable=too-many-locals,too-many-arguments
        ...
        Args:
            ...
            crop_width, crop_height: Crop rectangle width and height
            parent_bounds: Parent element's bounds
        Returns:
            Tuple of (left, top, width, height) in inches
        """
        width = crop_width if crop_width > 0 else original_width
        height = crop_height if crop_height > 0 else original_height
    ...
    def calculate_image_fit(self, image_width: float, image_height: float,  # pylint: disable=too-many-arguments
    ...
    def debug_info(self) -> str:
        ...
        return f"""
                ...
                - Slide Size: {self.slide_width_inches}" x {self.slide_height_inches}"
                ({self.slide_width_points} x {self.slide_height_points} pts)
                - Scale Factors: X={self.scale_x:.4f}, Y={self.scale_y:.4f},
                Uniform={self.uniform_scale:.4f}
