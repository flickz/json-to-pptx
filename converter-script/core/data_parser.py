...
class DataParser:
    ...
    def print_hierarchy(self, widget: Optional[Widget] = None, indent: int = 0):
        ...
        if isinstance(widget, TextWidget):
            ...
            if widget.resource:
                print(f"{indent_str}  Dimensions: {widget.resource.width:.0f} x "
                      f"{widget.resource.height:.0f}")

        if widget.position:
            print(f"{indent_str}  Position: ({widget.position.x:.2f}, "
                  f"{widget.position.y:.2f}) [{widget.position.schema}]")
