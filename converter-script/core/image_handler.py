...
class ImageHandler:
    ...
    def download_image(self, url: str, force_download: bool = False) -> Optional[Path]:
        ...
        try:
            ...
            logger.info(f"Image downloaded successfully: {cache_path.name}")
            return cache_path
    ...
    def get_image_info(self, image_path: Path) -> Optional[Tuple[int, int]]:
        ...
        try:
            ...
            logger.error(f"Error reading image info: {str(e)}")
            return None
        ...
        try:
            ...
            with Image.open(image_path) as img:
                ...
                if output_path is None:
                    ...
                    output_path = self.cache_dir / f"{stem}_crop_{int(crop_width)}x{int(crop_height)}{ext}"
                
                cropped.save(output_path, quality=95)
                logger.info(f"Image cropped and saved to: {output_path.name}")
